...
import { IoClient } from '../src/io-client.ts';
import { ioRouter } from '../src/io-router.ts';
...

  {
    ...
    return describe('Io Conformance', async () => {
      beforeEach(async () => {
        ...
        console.log(`Listener count: ${server.listenerCount}`);

        // Create an instance of the IoClient
        io = new IoClient(port);
        await io.init();
        await io.isReady();
